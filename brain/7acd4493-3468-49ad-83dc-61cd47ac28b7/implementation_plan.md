...
- **Navbar**: Remove the "Status" nav item.
- **Borrower Modal**: Add a status filter dropdown (ALL, YES, NO) next to the "Borrower List" heading inside the modal.

### Logic
#### [MODIFY] [app.js](file:///c:/Users/kanam/OneDrive/Antigravity%20Codes/interest-calculator/public/js/app.js)
- Update status filter selector to find the new dropdown in the modal.
- Ensure the dropdown only appears when the user is logged in (already handled by modal visibility).
- Update the `updateAuthUI` to remove references to the old navbar status container.
